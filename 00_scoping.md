...
**6. Freight Ratio**
The ratio of `freight_value` to `price` per order item, averaged at the seller level. This normalizes freight cost relative to product value, making sellers in different price tiers comparable. A freight ratio above 0.40 (the empirical p75 of seller-level average freight ratios, as determined in Phase 2 EDA) is used as the threshold for high-freight burden. The original 0.3 placeholder was revised after EDA showed it flags 40.5% of sellers — nearly the median, not an outlier signal. The p75 threshold marks the top 25% of sellers by freight burden and is the operationally meaningful cut: one in four sellers charging disproportionate freight relative to product price. The p90 (0.61) is available as a stricter "extreme" flag for sensitivity analysis in Phase 4.
